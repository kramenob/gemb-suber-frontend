		...
		<div :class="cn([ 'flex flex-col space-y-4', ])">
			...
			<div :class="cn([
				...
				>
					...
					<span :class="cn([ 'block', 'w-full', 'text-right font-semibold', ])">{{ i.value }}<span :class="cn([ 'sr-only', ])">{{ i === about[about.length - 1] ? '.' : ';' }}</span></span>
				</div>
			</div>
		</div>
		<!-- bottom -->
		<div :class="cn([ 'flex flex-col space-y-4', ])">
			...
			<div :class="cn([
				...
				<Button
					...
					@click="$emit('pay', data)"
					:disabled="data.paid"
				>{{ data.paid ? 'Договор оплачен' : 'Оплатить договор' }}</Button>
			</div>
		</div>
	</div>
</template>
